export function calculateProjectState(config, githubIssues) {
  ...
  for (const cycle of findCycles(byNumber)) errors.push(`依存循環: ${cycle.map((n) => `#${n}`).join(" → ")}`);
  ...
  for (const issue of executions) {
    ...
    if (issue.meta.manual_state === "blocked" || issue.unresolved.length > 0) issue.status = "BLOCKED";
  ...
  for (const milestone of sortedMilestones) {
    const required = executions.filter((issue) => issue.meta.milestone === milestone.id && issue.meta.required !== false);
    ...
    else if (started && unfinished.length > 0 && actionable.length === 0) status = "BLOCKED";
