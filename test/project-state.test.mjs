...
function config(manualState = null) {
  return {
    categories:["UX","APP","PLATFORM","DATA","VERIFY","OPS"].map((id)=>({id})),
    milestones:[{id:"M0",label:"今日を決める",order:0}],
    issues:[
      {number:2,milestone:"M0",category:"UX",required:true,depends_on:[],manual_state:manualState},
...
test("initial: UXだけReady、依存待ちはWaiting",()=>{
  const r=calculateProjectState(config(),gh());
  assert.equal(r.milestoneStates.get("M0"),"NOT STARTED");
  assert.deepEqual(
    Object.fromEntries(r.executions.map(i=>[i.number,i.status])),
    {2:"READY",3:"WAITING",4:"WAITING",5:"WAITING"}
  );
});
...
test("manual blockerはWaitingではなくBlocked",()=>{
  const r=calculateProjectState(config("blocked"),gh());
  assert.equal(r.executions.find(i=>i.number===2).status,"BLOCKED");
});
